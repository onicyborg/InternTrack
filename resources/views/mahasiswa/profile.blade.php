        ...
        <div class="col-lg-4">
            <div class="card h-100">
                <div class="card-body d-flex flex-column align-items-center text-center w-100">
                    @php
                        $photo =
                            optional(optional($user)->profile)->photo_url ?? asset('assets/media/avatars/blank.png');
        ...
        <div class="col-lg-8">
            <div class="card h-100">
                <div class="card-header">
                    <h3 class="card-title m-0">Profil Mahasiswa</h3>
                </div>
                <div class="card-body">
                    <form id="profileForm">
                        @csrf
...
@section('extra_js')
    <script>
        ...
        const routes = {
            // Gunakan fallback kosong jika route belum tersedia agar tidak error
            updateProfile: '{{ Route::has('mahasiswa.profile.update') ? route('mahasiswa.profile.update') : '' }}',
            updatePhoto: '{{ Route::has('mahasiswa.profile.photo') ? route('mahasiswa.profile.photo') : '' }}',
            changePassword: '{{ Route::has('mahasiswa.profile.change_password') ? route('mahasiswa.profile.change_password') : '' }}',
        };
        ...
        function ensureRoute(url) {
            if (!url) {
                window.toastr?.error?.('Route belum tersedia. Silakan hubungi admin.');
                return false;
            }
            return true;
        }
        ...
        photoInput?.addEventListener('change', function() {
            ...
            fetch(routes.updatePhoto, {
                ...
                const url = data?.data?.profile?.photo_url;
