...
@section('title', 'Pembina - Detail Attendance Mahasiswa')
@section('page_heading', 'Detail Attendance Mahasiswa (Pembina)')
...
@section('content')
    ...
    <div class="card mb-6">
        <div class="card-body">
            ...
            <div class="mt-4">
                <a href="{{ route('pembina.attendance.index') }}" class="btn btn-light">Kembali</a>
            </div>
        </div>
    </div>
                        ...
                        @foreach($rows as $row)
                            <tr>
                                ...
                                <td>
                                    ...
                                    <button class="btn btn-sm btnView {{ ($row['approve_pembina'] ?? '') === 'Approve' ? 'btn-success' : 'btn-light-primary' }}" {{ $disabled }}
                                        ...
                                        data-approved="{{ ($row['approve_pembina'] ?? '') === 'Approve' ? '1' : '0' }}"
                                        data-checkin-photo="{{ $row['photo_checkin_url'] ?? '' }}"
                                        data-checkin-ttd="{{ $row['ttd_checkin_url'] ?? '' }}"
                                        data-checkout-photo="{{ $row['photo_checkout_url'] ?? '' }}"
                                        data-checkout-ttd="{{ $row['ttd_checkout_url'] ?? '' }}">{{ ($row['approve_pembina'] ?? '') === 'Approve' ? 'Lihat Foto' : 'Verifikasi' }}</button>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
...
@section('extra_js')
    <script>
        (function(){
            ...
            const approveUrlTemplate = "{{ route('pembina.attendance.approve', ['attendanceId' => 'ATTENDANCE_ID']) }}";
                ...
                fetch(url, {
                  ...
                  .then(data => {
                      if (data && data.success) {
                          ...
                          // Update badge approve pembina (kolom index 5)
                          if (currentRowEl) {
                              const badgeCell = currentRowEl.querySelectorAll('td')[5];
                              if (badgeCell) {
