...
@section('title', 'Pembina - Detail Logbook Mahasiswa')
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex gap-2">
                                        @php $isPending = strtolower($apPemb) === 'pending'; @endphp
                                        @php $isReject = strtolower($apPemb) === 'reject'; @endphp
                                        <a href="#"
                                           ...
                                           data-status-pembina="{{ strtolower($apPemb) }}">
    ...
    <script>
        (function(){
            ...
            $(document).on('click', '.btnViewLogbook', function(e){
                ...
                // Fetch detail
                const detailUrl = "{{ route('pembina.logbooks_approval.detail', ['userId' => 'USER_ID', 'logbookId' => 'LOGBOOK_ID']) }}".replace('USER_ID', encodeURIComponent(userId)).replace('LOGBOOK_ID', encodeURIComponent(logbookId));
                $.get(detailUrl, function(resp){
                    ...
                    setEditorContent('rv_remark_pembina', d.remark_pembina || '');
                    const status = (d.approval_pembina || '').toLowerCase();
                    ...
                    // ZIP all link
                    zipBtn.href = "{{ route('pembina.logbooks_approval.download_zip', ['logbookId' => 'LOGBOOK_ID']) }}".replace('LOGBOOK_ID', encodeURIComponent(logbookId));
            ...
            document.getElementById('btnSubmitDecision').addEventListener('click', function(){
                ...
                const remark = (window.tinymce && tinymce.get('rv_remark_pembina')) ? tinymce.get('rv_remark_pembina').getContent() : (document.getElementById('rv_remark_pembina').value || '');
                ...
                const url = "{{ route('pembina.logbooks_approval.decide', ['logbookId' => 'LOGBOOK_ID']) }}".replace('LOGBOOK_ID', encodeURIComponent(currentLogbookId));
                $.ajax({
                    url: url,
                    method: 'POST',
